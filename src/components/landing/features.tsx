...
function FeatureVisual({ visual }: { visual: (typeof features)[number]['visual'] }) {
  switch (visual) {
    ...
    case 'cards':
      return (
        <div className="relative flex min-h-[13rem] items-center justify-center overflow-hidden rounded-[1.5rem] border border-warm-200 bg-white">
          ...
          <div className="relative z-10 h-40 w-60 rounded-[1.6rem] border border-warm-200 bg-white px-5 py-4 shadow-[0_20px_48px_-36px_rgba(15,23,42,0.75)]">
            <div className="text-xs font-medium uppercase tracking-[0.22em] text-warm-400">
              Card preview
            </div>
            <div className="mt-5 text-base font-semibold leading-snug text-warm-800">
              What makes spaced repetition effective for exam prep?
            </div>
            <div className="mt-5 h-2 w-2/3 rounded-full bg-warm-200" />
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6">
        <div id="simple-by-design" className="mx-auto max-w-3xl scroll-mt-28 text-center">
          ...
          <p className="mt-6 text-lg leading-relaxed text-warm-500">
            Everything important lives in one place, with just enough structure to keep studying
            moving and none of the usual interface noise.
          </p>
        </div>
          ...
          {features.map((feature) => {
            ...
            return (
              <article
                key={feature.title}
                className={cn(
                  'group relative overflow-hidden rounded-[2rem] border p-6 shadow-[0_30px_80px_-48px_rgba(15,23,42,0.35)] transition-transform duration-300 hover:-translate-y-1',
                  styles.shell,
                  feature.className
                )}
              >
